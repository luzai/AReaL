...
REFINER_TEMPLATE = (
    "You are tasked with revising a draft solution to a problem based on the critique provided. "
    "Please provide a revised solution that addresses the feedback and improves the overall quality of the solution.\n\n"
    "Problem: $question\n\n"
    "Solution: $generator\n\n"
    "Critique: $verifier\n\n"
    "Please reason step by step, and put your final answer within \\boxed{}."
...
class OpenAIAgentWorkflow(RolloutWorkflow):
    def __init__(
        ...
        # verifier agent
        self.verifier = OpenAIAgentWrapper(
            temperature=gconfig.temperature,
            max_tokens=gconfig.max_tokens,
            agent_name="verifier",
            reward_fn_path=reward_fn_path,
        ...
        # refiner agent
        self.refiner = OpenAIAgentWrapper(
            temperature=gconfig.temperature,
            max_tokens=gconfig.max_tokens,
            agent_name="refiner",
            reward_fn_path=reward_fn_path,
    ...
    async def arun_episode(self, engine, data: dict[str, Any]):
        ...
        generator_output, _ = await self._run_stage(
        ...
        verifier_prompt = render_template(
            VERIFIER_TEMPLATE, question=question, generator=generator_output
        )
        verifier_output, _ = await self._run_stage(
            wrapper=self.verifier,
            client=client,
            data=data,
        ...
        refiner_prompt = render_template(
            REFINER_TEMPLATE,
            question=question,
            generator=generator_output,
            verifier=verifier_output,
        )
        refiner_output, final_reward = await self._run_stage(
            wrapper=self.refiner,
            client=client,
            data=data,
            prompt=refiner_prompt,
            norm_group="group_3",
        )
        logger.debug("Refiner output: %s", refiner_output)

        # equally distribute rewards
        for interaction in client._cache.values():
...
def main(args):
    ...
    with PPOTrainer(
        ...
        trainer.train(
            workflow="examples.openai_agents.train_math_marti_shared.OpenAIAgentWorkflow",
            workflow_kwargs=workflow_kwargs,
            eval_workflow="examples.openai_agents.train_math_marti_shared.OpenAIAgentWorkflow",
