...
DP_SEQUENCE_PADDING_MARKER = "_areal_pad_uneven_dp_sequences"
DP_VALID_TOKEN_MASK = "_dp_valid_token_mask"
...
class MicroBatchList:
    ...
    # Data-parallel padding information. A shorter rank can append zero-loss
    # sequences so all ranks execute the same number of synchronized forwards.
    valid_batch_size: int | None = None
    padded_batch_size: int | None = None
    dp_padding_source_indices: list[int] | None = None
    ...
    def to(self, *args, **kwargs):
        ...
        return MicroBatchList(
            ...
            dp_padding_source_indices=self.dp_padding_source_indices,
        )
...
def _pad_batch_rows_to_dp_max(
    data: dict[str, Any],
    granularity: int,
    group: dist.ProcessGroup | None = None,
) -> tuple[dict[str, Any], int, list[int]]:
    """Append zero-loss rows until every DP rank has the same batch size.

    The synchronized micro-batch allocator requires every rank to be able to
    create the maximum number of micro-batches requested by any peer. Agentic
    rollouts can produce different numbers of decision sequences per rank, so
    a shorter rank otherwise fails before forward with ``values < min_groups``.

    Dummy rows duplicate valid model inputs, which keeps every forward well
    formed, while their ``loss_mask`` is zeroed so they contribute neither
    loss nor gradients. Forward-only callers remove the dummy outputs using
    the metadata carried by :class:`MicroBatchList`.
    """
    attention_mask = data["attention_mask"]
    if attention_mask.ndim != 2:
        raise ValueError("Attention mask must be a 2D tensor.")
    local_bs = int(attention_mask.shape[0])
    if local_bs == 0:
        raise RuntimeError("Cannot data-parallel pad an empty batch.")
    if not dist.is_initialized():
        return data, local_bs, []

    all_batch_sizes: list[int | None] = [
        None for _ in range(dist.get_world_size(group))
    ]
    dist.all_gather_object(all_batch_sizes, local_bs, group=group)
    if any(size is None for size in all_batch_sizes):
        raise RuntimeError(f"Incomplete DP batch-size gather: {all_batch_sizes}")
    concrete_batch_sizes = [int(size) for size in all_batch_sizes if size is not None]
    target_bs = max(concrete_batch_sizes)
    if len(set(concrete_batch_sizes)) == 1:
        return data, local_bs, []
    if "loss_mask" not in data:
        raise RuntimeError(
            "Uneven DP sequence batches require loss_mask so padding can be "
            "guaranteed to contribute zero loss."
        )
    if local_bs % granularity != 0 or target_bs % granularity != 0:
        raise RuntimeError(
            f"DP batch sizes {all_batch_sizes} must divide granularity {granularity}."
        )

    pad_count = target_bs - local_bs
    source_group = list(range(local_bs - granularity, local_bs))
    source_indices = [source_group[i % granularity] for i in range(pad_count)]
    padded: dict[str, Any] = {}
    for key, value in data.items():
        if torch.is_tensor(value) and value.ndim >= 1 and value.shape[0] == local_bs:
            source = value[source_indices].clone()
            if key in {"loss_mask", DP_VALID_TOKEN_MASK}:
                source.zero_()
            padded[key] = torch.cat((value, source), dim=0)
        elif is_multi_modal_key(key):
            if not isinstance(value, list) or len(value) != local_bs:
                raise RuntimeError(
                    f"{key} must be a list with {local_bs} batch entries before "
                    "DP sequence padding."
                )
            padded[key] = [
                *value,
                *(copy.copy(value[index]) for index in source_indices),
            ]
        else:
            padded[key] = value

    if DP_VALID_TOKEN_MASK not in padded:
        valid_token_mask = attention_mask.to(dtype=torch.bool)
        dummy_token_mask = torch.zeros(
            (pad_count, *valid_token_mask.shape[1:]),
            dtype=torch.bool,
            device=valid_token_mask.device,
        )
        padded[DP_VALID_TOKEN_MASK] = torch.cat(
            (valid_token_mask, dummy_token_mask), dim=0
        )

    padded_loss_mask = padded["loss_mask"]
    if not torch.is_tensor(padded_loss_mask) or padded_loss_mask.shape[0] != target_bs:
        raise RuntimeError("loss_mask was not padded along the batch dimension.")
    logger.warning(
        "DP_SEQUENCE_PAD_AUDIT "
        f"rank={dist.get_rank(group)} local_batch_size={local_bs} "
        f"target_batch_size={target_bs} dummy_sequences={pad_count} "
        f"all_batch_sizes={all_batch_sizes}"
    )
    return padded, local_bs, source_indices
...
def split_padded_tensor_dict_into_mb_list(
    ...
    mb_spec: MicroBatchSpec,
    group: dist.ProcessGroup | None = None,
    pad_uneven_dp_sequences: bool = False,
) -> MicroBatchList:
    ...
    if pad_uneven_dp_sequences:
        data, valid_batch_size, dp_padding_source_indices = _pad_batch_rows_to_dp_max(
            data, granularity, group=group
        )
    else:
        valid_batch_size = int(data["attention_mask"].shape[0])
        dp_padding_source_indices = []
    ...
    return MicroBatchList(
        ...
        dp_padding_source_indices=dp_padding_source_indices,
    )
...
def bcast_mb_list(
    ...
    to_broadcast = (
        [
            ...
            mb_list.valid_batch_size,
            mb_list.padded_batch_size,
            mb_list.dp_padding_source_indices,
        ]
        if mb_list
