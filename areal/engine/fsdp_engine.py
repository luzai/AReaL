...
class FSDPEngine(TrainEngine):
    ...
    def initialize(self, addr: str | None, ft_spec: FinetuneSpec, *args, **kwargs):
        ...
        if os.getenv("MAAPACMAN_STRICT_CPU_STAGE") == "1":
            post_shard_bad: list[str] = []
            post_shard_devices: dict[str, int] = {}
            expected_type = (
                "cpu" if self.cpu_offload is not None else current_platform.device_type
            )
            for name, value in self.model.named_parameters():
                local = value.to_local() if isinstance(value, DTensor) else value
                device = str(local.device)
                post_shard_devices[device] = post_shard_devices.get(device, 0) + 1
                if local.is_meta or local.device.type != expected_type:
                    post_shard_bad.append(
                        f"{name}:{local.device}:meta={local.is_meta}:expected={expected_type}"
                    )
            if post_shard_bad:
                raise RuntimeError(
                    "distributed/sharded checkpoint placement failed: "
                    + ", ".join(post_shard_bad[:20])
                )
            self.logger.info(
                "ITER16_DISTRIBUTED_SHARD_AUDIT "
                + json.dumps(
                    {
                        "rank": dist.get_rank(),
                        "devices": post_shard_devices,
                        "native_cpu_offload": self.cpu_offload is not None,
                        "all_non_meta": True,
                    },
                    sort_keys=True,
                )
    ...
    def destroy(self):
        # torch_memory_saver cannot safely free CUDA allocations while its
        # allocator is paused.  Deleting an offloaded model in that state can
        # make the worker exit inside the TMS free hook before the HTTP destroy
        # RPC returns.  Resume the whole DP group first, then release the model
        # and optimizer normally.
        if self.is_offload:
            self.logger.info("TMS_DESTROY_ONLOAD_AUDIT phase=before_onload")
            self.onload()
            self.logger.info("TMS_DESTROY_ONLOAD_AUDIT phase=after_onload")
    ...
    @staticmethod
    def _audit_local_tensor(value: torch.Tensor) -> torch.Tensor:
        return value.to_local() if isinstance(value, DTensor) else value

    @staticmethod
    def _audit_rng_digest(value: object) -> str:
        return hashlib.sha256(pickle.dumps(value, protocol=5)).hexdigest()

    def runtime_state_audit(self, expected_phase: str = "cuda") -> dict[str, Any]:
        """Fail closed on model/optimizer placement and record reconstructed state."""
        if expected_phase not in {"cuda", "cpu"}:
            raise ValueError(f"unsupported audit phase: {expected_phase}")
        expected_type = (
            current_platform.device_type if expected_phase == "cuda" else "cpu"
        )
        ...
        for kind, named_values in (
            ...
            for name, value in named_values:
                local = self._audit_local_tensor(value)
                if kind == "parameter":
                    parameter_count += 1
                    parameter_numel += local.numel()
                else:
                    buffer_count += 1
                devices[str(local.device)] = devices.get(str(local.device), 0) + 1
                if local.is_meta or local.device.type != expected_type:
                    bad_devices.append(
                        f"{kind}:{name}:{local.device}:meta={local.is_meta}"
                    )
                if local.numel() and (local.is_floating_point() or local.is_complex()):
                    flat = local.detach().reshape(-1)
                    sample = flat[:: max(1, flat.numel() // 17)][:17]
                    if not torch.isfinite(sample).all().item():
                        nonfinite_samples.append(f"{kind}:{name}")

        optimizer_state_tensors = 0
        optimizer_state_devices: dict[str, int] = {}
        optimizer_bad_devices: list[str] = []
        for param, state in self.optimizer.state.items():
            param_local = self._audit_local_tensor(param)
            for state_name, value in state.items():
                if not isinstance(value, torch.Tensor):
                    continue
                local = self._audit_local_tensor(value)
                optimizer_state_tensors += 1
                optimizer_state_devices[str(local.device)] = (
                    optimizer_state_devices.get(str(local.device), 0) + 1
                )
                if local.device != param_local.device:
                    optimizer_bad_devices.append(
                        f"{state_name}:{local.device}:param={param_local.device}"
                    )
                if local.numel() and (local.is_floating_point() or local.is_complex()):
                    flat = local.detach().reshape(-1)
                    sample = flat[:: max(1, flat.numel() // 17)][:17]
                    if not torch.isfinite(sample).all().item():
                        nonfinite_samples.append(f"optimizer:{state_name}")

        if bad_devices or optimizer_bad_devices or nonfinite_samples:
            raise RuntimeError(
                "runtime state audit failed: "
                + json.dumps(
                    {
                        "bad_devices": bad_devices[:20],
                        "optimizer_bad_devices": optimizer_bad_devices[:20],
                        "nonfinite_samples": nonfinite_samples[:20],
                    },
                    sort_keys=True,
                )
            )
        ...
        if torch.cuda.is_available():
            cuda_rng = hashlib.sha256(
                torch.cuda.get_rng_state(self.device).cpu().numpy().tobytes()
            ).hexdigest()
        report = {
            "rank": dist.get_rank(),
            ...
            "rng": {
                "python": self._audit_rng_digest(random.getstate()),
                "numpy": self._audit_rng_digest(np.random.get_state()),
        ...
        self.logger.info("RUNTIME_STATE_AUDIT " + json.dumps(report, sort_keys=True))
        return report
    ...
    def _create_device_model(self):
        ...
        if os.getenv("MAAPACMAN_STRICT_CPU_STAGE") == "1":
            ...
            for name, value in list(model.named_parameters()) + list(
                ...
                cpu_stage_tensors += 1
                cpu_stage_numel += value.numel()
                if value.is_meta or value.device.type != "cpu":
                    cpu_stage_bad.append(f"{name}:{value.device}:meta={value.is_meta}")
            if cpu_stage_bad:
                raise RuntimeError(
                    "strict CPU checkpoint staging failed: "
                    + ", ".join(cpu_stage_bad[:20])
                )
            self.logger.info(
                "ITER16_CPU_STAGE_AUDIT "
                + json.dumps(
                    {
                        "rank": dist.get_rank(),
                        "tensor_count": cpu_stage_tensors,
                        "numel": cpu_stage_numel,
                        "all_cpu": True,
                        "all_non_meta": True,
    ...
    def _prepare_mb_list(self, input_: dict[str, Any]) -> MicroBatchList:
        ...
        input_ = input_.copy()
        pad_uneven_dp_sequences = bool(input_.pop(DP_SEQUENCE_PADDING_MARKER, False))
        ...
        mb_list = split_padded_tensor_dict_into_mb_list(
            ...
            self.config.mb_spec,
            group=self.dp_group,
            pad_uneven_dp_sequences=pad_uneven_dp_sequences,
        )
        ...
        for i, mb in enumerate(mb_list.padded_mbs):
            ...
            mb_list.padded_mbs[i].pop(DP_VALID_TOKEN_MASK, None)
...
class FSDPPPOActor(FSDPEngine):
    ...
    def safety_probe_backward(self, *args, **kwargs) -> dict[str, Any]:
        """Exercise the production PPO backward path without changing state."""
        if not torch.cuda.is_available():
            raise RuntimeError("safety probe requires CUDA")

        def parameter_fingerprint() -> str:
            digest = hashlib.sha256()
            for name, value in self.model.named_parameters():
                local = self._audit_local_tensor(value).detach().reshape(-1)
                digest.update(name.encode("utf-8"))
                digest.update(str(tuple(local.shape)).encode("ascii"))
                digest.update(str(local.dtype).encode("ascii"))
                if local.numel():
                    sample = local[:: max(1, local.numel() // 31)][:31]
                    digest.update(sample.float().cpu().contiguous().numpy().tobytes())
            return digest.hexdigest()

        before_weight_fingerprint = parameter_fingerprint()
        before_optimizer_entries = len(self.optimizer.state)
        before_scheduler = copy.deepcopy(self.lr_scheduler.state_dict())
        before_version = self.get_version()
        torch.cuda.reset_peak_memory_stats(self.device)
        audit_records: list[dict[str, Any]] = []
        original_optimizer_step = self.optimizer_step

        def backward_only_optimizer_step() -> dict[str, float]:
            grad_tensors = 0
            grad_nonzero = 0
            grad_sum_sq = 0.0
            bad_grad_devices: list[str] = []
            ...
            for name, param in self.model.named_parameters():
                ...
                if grad is None:
                    continue
                grad_local = self._audit_local_tensor(grad)
                grad_tensors += 1
                if grad_local.device != param_local.device:
                    bad_grad_devices.append(
                        f"{name}:grad={grad_local.device}:param={param_local.device}"
                    )
                if not torch.isfinite(grad_local).all().item():
                    nonfinite_grads.append(name)
                if torch.count_nonzero(grad_local).item() > 0:
                    grad_nonzero += 1
                grad_sum_sq += float(
                    torch.sum(grad_local.detach().float().square()).item()
                )
            if (
                bad_grad_devices
                or nonfinite_grads
                ...
                raise RuntimeError(
                    "backward-only safety audit failed: "
                    + json.dumps(
                        {
                            "bad_grad_devices": bad_grad_devices[:20],
                            "nonfinite_grads": nonfinite_grads[:20],
            ...
            record = {
                "grad_tensors": grad_tensors,
                "grad_nonzero": grad_nonzero,
                "local_grad_norm": math.sqrt(grad_sum_sq),
            }
            audit_records.append(record)
            self.optimizer_zero_grad()
            return {
                "update_successful": 0.0,
                "grad_norm": record["local_grad_norm"],
                "lr": float(self.lr_scheduler.get_last_lr()[0]),
            }

        self.optimizer_step = backward_only_optimizer_step
        try:
            self.actor.ppo_update(*args, **kwargs)
        finally:
            self.optimizer_step = original_optimizer_step

        after_weight_fingerprint = parameter_fingerprint()
        after_scheduler = self.lr_scheduler.state_dict()
        after_optimizer_entries = len(self.optimizer.state)
        after_version = self.get_version()
        if before_weight_fingerprint != after_weight_fingerprint:
            raise RuntimeError("safety probe changed actor weights")
        if before_optimizer_entries != after_optimizer_entries:
            raise RuntimeError("safety probe changed optimizer state")
        if before_scheduler != after_scheduler:
            raise RuntimeError("safety probe changed scheduler state")
        if before_version != after_version:
            raise RuntimeError("safety probe changed actor version")

        report = {
            "rank": dist.get_rank(),
            "backward_calls": len(audit_records),
            "records": audit_records,
            "weight_fingerprint": after_weight_fingerprint,
            "optimizer_state_entries": after_optimizer_entries,
        ...
        self.logger.info(
            "SAFETY_PROBE_BACKWARD_AUDIT " + json.dumps(report, sort_keys=True)
        )
        return report
