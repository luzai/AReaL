...
def reorder_and_pad_outputs(
    ...
    """
    valid_batch_size = mb_list.valid_batch_size or len(output_seqlens)
    padded_batch_size = mb_list.padded_batch_size or valid_batch_size
    if mb_list.forward_indices is None or mb_list.backward_indices is None:
        raise RuntimeError("Micro-batch reorder indices are missing.")
    if len(mb_list.forward_indices) != padded_batch_size:
        raise RuntimeError(
            "Forward reorder metadata does not match the padded batch size."
        )
    if len(mb_list.backward_indices) != padded_batch_size:
        raise RuntimeError(
            "Backward reorder metadata does not match the padded batch size."
        )
    if len(output_seqlens) != valid_batch_size:
        raise RuntimeError(
            "Forward output sequence metadata does not match the valid batch size: "
            f"{len(output_seqlens)} != {valid_batch_size}."
        )
    padded_output_seqlens = list(output_seqlens)
    if padded_batch_size > valid_batch_size:
        source_indices = mb_list.dp_padding_source_indices or []
        if len(source_indices) != padded_batch_size - valid_batch_size:
            raise RuntimeError(
                "DP padding source metadata does not match the padded batch size."
            )
        padded_output_seqlens.extend(output_seqlens[i] for i in source_indices)

    res = aggregate_fn(outputs)
    seqlens = [padded_output_seqlens[i] for i in mb_list.forward_indices]
    unpacked = unpack_sequence(res, lens=seqlens, dim=0)
    reordered = reorder_list(unpacked, mb_list.backward_indices)
    reordered = reordered[:valid_batch_size]
