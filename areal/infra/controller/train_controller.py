                ...
                async def _destroy_all_engines():
                    tasks = [
                        self.scheduler.async_call_engine(
                            ...
                            method="destroy",
                            engine_name=self._engine_name(rank),
                            # TMS may have paused/offloaded the engine before
                            # teardown.  Broadcasting even an empty RPC payload
                            # would allocate a CUDA tensor on that paused worker
                            # and can fail before engine.destroy() is reached.
