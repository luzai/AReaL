...
class PPOTrainer:
    ...
    def _compute_logp_in_rpc_chunks(self, engine, rollout_batch, role: str):
        """Compute log-probabilities in DP-aligned controller RPC chunks.

        The controller RPC guard materializes its full input shard on CUDA before
        the worker starts forwarding microbatches.  Keeping a 48-rollout batch in
        one RPC can therefore exhaust CUDA virtual-memory allocations even when
        ``max_tokens_per_mb`` is small.  Chunking here, before RPC localization,
        bounds that residency without changing rollout order or PPO semantics.
        """

        total = len(rollout_batch)
        chunk_size = int(os.getenv("MAAPACMAN_LOGP_RPC_CHUNK_SIZE", "0"))
        if chunk_size == 0:
            return engine.compute_logp(rollout_batch)
        if chunk_size < 0:
            raise ValueError(f"negative logp RPC chunk size: {chunk_size}")

        # TrainController is itself rank 0 of a one-process controller world, so
        # ``data_parallel_world_size`` intentionally reports 1 there.  The RPC
        # dispatch contract must instead use the engine allocation's real DP
        # degree (4 for the current H800 actor/reference allocations).
        dp_size = int(engine.parallel_strategy.dp_size)
        if chunk_size % dp_size != 0:
            raise ValueError(
                "logp RPC chunk size must be a multiple of the engine DP size: "
                f"chunk_size={chunk_size} dp_size={dp_size}"
            )
        if total == 0 or total % dp_size != 0:
            raise ValueError(
                "logp RPC input count must be nonzero and divisible by DP size: "
                f"count={total} dp_size={dp_size}"
            )

        outputs = []
        chunk_count = (total + chunk_size - 1) // chunk_size
        for chunk_index, start in enumerate(range(0, total, chunk_size), start=1):
            chunk = rollout_batch[start : start + chunk_size]
            if len(chunk) % dp_size != 0:
                raise ValueError(
                    "final logp RPC chunk is not DP aligned: "
                    f"chunk_index={chunk_index} size={len(chunk)} dp_size={dp_size}"
                )
            chunk_outputs = engine.compute_logp(chunk)
            if chunk_outputs is None or len(chunk_outputs) != len(chunk):
                raise RuntimeError(
                    "logp RPC chunk returned the wrong result count: "
                    f"role={role} chunk_index={chunk_index} "
                    f"expected={len(chunk)} actual="
                    f"{None if chunk_outputs is None else len(chunk_outputs)}"
                )
            outputs.extend(chunk_outputs)
            logger.info(
                "LOGP_RPC_CHUNK_AUDIT role=%s chunk=%d/%d size=%d "
                "completed=%d total=%d dp_size=%d",
                role,
                chunk_index,
                chunk_count,
                len(chunk),
        ...
        for global_step in range(start_step, max_steps):
            ...
            if self.ref is not None:
                ...
                with (
                    ...
                    ref_logps = self._compute_logp_in_rpc_chunks(
                        self.ref, rollout_batch, role="ref"
                    )
            ...
            if config.actor.should_compute_prox_logp():
                with (
                    ...
                    prox_logps = self._compute_logp_in_rpc_chunks(
                        self.actor, rollout_batch, role="actor"
                    )
            ...
            gate_dir = os.getenv("MAAPACMAN_FIRST_UPDATE_GATE_DIR")
            gate_step = int(os.getenv("MAAPACMAN_FIRST_UPDATE_GATE_GLOBAL_STEP", "16"))
            if gate_dir and global_step == gate_step:
