...
def test_ppo_batch_padding_clones_valid_input_with_zero_loss_mass():
    ...
    data = {
        ...
        "episode_loss_weights": torch.tensor([[0.5, 0.5, 0.5], [0.5, 0.5, 0.0]]),
    ...
    torch.testing.assert_close(padded["attention_mask"][2], data["attention_mask"][1])
...
def test_dp4_nondivisible_ppo_rows_are_feasible_after_local_padding():
    ...
    for split in splits:
        ...
        data = {
            ...
            "loss_mask": torch.ones(local_count, sequence_length, dtype=torch.bool),
        ...
        assert padded["episode_loss_weights"][local_count:].count_nonzero() == 0
...
def test_controller_rejects_user_supplied_ppo_padding_target():
    ...
    controller = object.__new__(TrainController)
    controller.train_alloc = SimpleNamespace(parallel=SimpleNamespace(dp_size=2))
    rows = [{"attention_mask": torch.ones(1, 2, dtype=torch.bool)} for _ in range(2)]
...
def test_gate3_repartition_reproduces_failure_and_balances_dp4():
    ...
    rank_lengths = [
        [int(item["attention_mask"].sum().item()) for item in split] for split in splits
    ]
